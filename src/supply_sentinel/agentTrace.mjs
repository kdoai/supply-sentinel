// agentTrace.mjs — deterministic multi-agent run trace generator.
//
// Supply Sentinel is presented as a *constrained* autonomous multi-agent system
// (OpenClaw-style autonomy, but with a small, locked tool surface). This module
// turns an already-computed dashboard model into a structured `agent_run`:
//   - agents[]          : the 7 named agents (orchestrator + 6 workers), in order
//   - tool_calls[]      : a flat timeline of the tools the agents invoked
//   - decisions[]       : what the AI may execute vs. what needs human approval
//   - blocked_evidence[]: external text flagged as prompt-injection and excluded
//
// IMPORTANT: this is a PURE function of the model. Every headline number it
// reports (risk score, affected ratio, spend-at-risk, inventory days) is read
// straight out of the model the propagation engine produced, so the trace can
// NEVER drift from the 82 / 65% / $7.8M / 5-day invariants. No network, no
// randomness — `web/js/agentTrace.js` is a byte-for-byte behavioural mirror so
// the browser timeline regenerates the same trace per month.

// ---- agent roster (display order) ------------------------------------------
...
// External-document instruction patterns. Risk Scout treats sources as *data*,
// never instructions; Evidence Verifier flags anything that reads like an
// injected command and drops it from the verified evidence set.
...
// Deterministic clock: base time + i*step seconds, formatted HH:MM:SS in the
// model's own timezone-naive way. Never uses Date.now so tests are stable.
...
export function buildAgentRun(model = {}, options = {}) {
  ...
  // Headline numbers — read straight from the engine output (never recomputed).
  ...
  // --- Evidence Verifier: scan every external source for injected commands ---
  ...
  // --- tool-call timeline (the "AI is really working" evidence) ---
  const toolCalls = [];
  let t = 0;
  ...
  const agents = [
    ...
    null, // decision_gate filled below (needs decisions[])
    agentRecord(AGENT_ROSTER[6], 19, t, "確定影響 + 起案", persisted ? "管理職レポートを生成し Cosmos DB に保存" : "管理職レポートを生成(ローカル保存)"),
  ];

  // --- Decision Gate: build the decision queue (human-approval + AI-auto) ---
  const decisions = [];
  ...
  // AI-executable (draft-only) items, to show the gate genuinely splitting work.
