  ...
  const model = {
    meta: {
      ...
      evidence_collection: {
        ...
        search_health: data.liveEvidence?.search_health ?? {
          provider: data.liveEvidence?.mode ?? "disabled",
          retrieved_count: 0,
          accepted_count: 0,
          rejected_count: 0,
          error_count: Array.isArray(data.liveEvidence?.errors) ? data.liveEvidence.errors.length : 0,
          errors: Array.isArray(data.liveEvidence?.errors) ? data.liveEvidence.errors : [],
...
function buildProvenance(data = {}) {
  ...
  for (const news of Array.isArray(data.newsEvents) ? data.newsEvents : []) {
    ...
    sources.push({
      ...
      url: news.url ?? null,
      canonical_url: news.canonical_url ?? news.url ?? null,
      feed_url: news.feed_url ?? null,
      title: news.headline ?? null,
      snippet: news.summary ?? null,
      source_domain: news.source_domain ?? null,
      provider: news.provider ?? (news.live ? "live_web" : "demo"),
      query: news.query ?? null,
      material: news.material ?? null,
      region: news.region ?? null,
      relevance_score: news.relevance_score ?? null,
      content_hash: news.content_hash ?? null,
      status: news.status ?? "accepted",
      evidence_id: news.evidence_id ?? news.id ?? null,
      published_at: news.published_at ?? null,
      fetched_at: news.fetched_at ?? null,
